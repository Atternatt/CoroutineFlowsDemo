...
import com.m2f.sherpanytest.coreBusiness.arch.data.repository.flow.*
...
fun <K, V> GetDataSource<V>.getAll(ids: List<K>): Flow<List<V>> =
    flow { emit(getAll(IdsQuery(ids))) }
...
fun <K, V> FlowPutDataSource<V>.putAll(ids: List<K>, values: List<V>?) =
    putAll(IdsQuery(ids), values)

fun <K, V> PutDataSource<V>.putAll(ids: List<K>, values: List<V>?): Flow<List<V>> = flow {
    emit(
      putAll(
        IdsQuery(ids), values
      )
...
fun <K> DeleteDataSource.deleteAll(ids: List<K>): Flow<Unit> =
    flow { emit(deleteAll(IdsQuery(ids))) }
...
fun <K, V> FlowGetDataSource<K>.toFlowGetRepository(mapper: Mapper<K, V>): FlowGetRepository<V> =
    toFlowGetRepository().withMapping(mapper)

fun <V> FlowPutDataSource<V>.toPutRepository() = SingleFlowPutDataSourceRepository(this)

fun <K, V> FlowPutDataSource<K>.toPutRepository(
  toMapper: Mapper<K, V>,
  fromMapper: Mapper<V, K>
): FlowPutRepository<V> = toPutRepository().withMapping(toMapper, fromMapper)
