...
suspend fun <K, V> GetRepository<V>.get(id: K, operation: Operation = DefaultOperation): V = get(
  IdQuery(id), operation)

suspend fun <K, V> GetRepository<V>.getAll(ids: List<K>, operation: Operation = DefaultOperation): List<V> = getAll(
  IdsQuery(ids), operation)

suspend fun <K, V> PutRepository<V>.put(id: K, value: V?, operation: Operation = DefaultOperation): V = put(
...
suspend fun <K> DeleteRepository.deleteAll(ids: List<K>, operation: Operation = DefaultOperation) = deleteAll(
  IdsQuery(ids), operation)
